...
class StructureWindowUdfTest extends FunSuite with DataFrameSuiteBase {
  test("too much events") {
    ...
    /* result */
    val actual =
      StructureWindowUdfTest.callWindowRdd(input)
        .select(col("ip"), col("aggregation.lastEvent").cast(LongType).as("last"), col("incident"))
        .distinct()
        .sort(col("last"), col("incident"))
